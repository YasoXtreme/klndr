...
function changeUsername(userId, newUsername) {
  ...
  if (db.users.some(u => u.id !== userId && u.username.toLowerCase() === cleanUsername)) {
...
function createTask(userId, taskData) {
  ...
  const newTask = {
    ...
    start_times: Array.isArray(taskData.start_times) ? taskData.start_times : [],
    durations: Array.isArray(taskData.durations) ? taskData.durations : [],
    total_duration: Number(taskData.total_duration || taskData.default_timing || 60),
    is_locked: taskData.is_locked !== undefined ? Boolean(taskData.is_locked) : true,
...
function updateTask(userId, taskId, updates) {
  ...
  if (updates.start_times !== undefined) updatedTask.start_times = updates.start_times;
  if (updates.durations !== undefined) updatedTask.durations = updates.durations;
  if (updates.total_duration !== undefined) updatedTask.total_duration = Number(updates.total_duration);
  if (updates.is_locked !== undefined) updatedTask.is_locked = Boolean(updates.is_locked);
  if (updates.completed !== undefined) updatedTask.completed = Boolean(updates.completed);
...
function batchUpdateTasks(userId, taskUpdatesList) {
  ...
  for (const item of taskUpdatesList) {
    const idx = db.tasks.findIndex(t => t.id === item.id && t.user_id === userId);
    if (idx !== -1) {
      db.tasks[idx] = {
        ...db.tasks[idx],
        ...item,
